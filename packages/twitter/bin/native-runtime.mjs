...
function binaryNameForPlatform(platform = process.platform) {
  return platform === "win32" ? "z0-twitter-cli.exe" : "z0-twitter-cli";
}
...
export function resolveNativeBinary(packageRoot, env = process.env) {
  const overridePath = ensureAbsolutePath(process.cwd(), env.Z0_TWITTER_CLI_BIN);
  const candidates = [
    overridePath,
  ...
  throw new Error(
    [
      "No Twitter CLI binary was found.",
      "Run `pnpm --filter @z0/twitter build` or set `Z0_TWITTER_CLI_BIN`.",
...
export function stageNativeBinary(packageRoot, options = {}) {
  ...
  if (!existsSync(sourcePath)) {
    throw new Error(`Twitter native binary not found at ${sourcePath}. Build the native target first.`);
  }
