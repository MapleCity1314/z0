...
const requestedKeys = (process.env.Z0_BILIBILI_RELEASE_TARGETS || `${process.platform}-${process.arch}`)
  .split(",")
...
for (const key of requestedKeys) {
  ...
  const buildResult = spawnSync(
    "cargo",
    ["build", "--manifest-path", manifestPath, "--bin", "z0-bilibili-cli", "--release", "--target", config.target],
    {
      cwd: packageRoot,
      stdio: "inherit",
      env: process.env,
