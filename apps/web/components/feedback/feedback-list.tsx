...
import { CheckCircle2, Circle, Clock, AlertCircle, XCircle, MessageSquare } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { ScrollArea } from "@/components/ui/scroll-area";
import { getFeedbackByUserId } from "@/lib/db/feedback-queries";
...
export async function FeedbackList({ userId }: { userId: string }) {
  ...
  const items = await getFeedbackByUserId(userId);
  ...
  if (items.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center space-y-3 rounded-xl border border-dashed border-zinc-800 bg-zinc-900/30 p-8 text-center">
        <div className="flex size-12 items-center justify-center rounded-full bg-zinc-900 border border-zinc-800">
  ...
  return (
    <ScrollArea className="h-full pr-4">
      <div className="space-y-4 pb-4">
        {items.map((feedback) => {
          const status = STATUS_CONFIG[feedback.status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.pending;
          ...
          return (
            ...
            >
              ...
              <div className={cn("absolute left-0 top-0 h-full w-1", status.bg.replace("/10", "/40"))} />
              ...
              <div className="pl-2 space-y-3">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="rounded-md border-zinc-800 bg-zinc-950 px-2 py-0.5 text-xs font-normal text-zinc-400">
                        {feedback.type}
                      </Badge>
                      {feedback.category && (
                    ...
                    <h3 className="text-sm font-medium text-zinc-100 leading-none">
                      {feedback.title}
                    </h3>
                  </div>
                  
                  <Badge variant="outline" className={cn("flex items-center gap-1.5 pl-1.5 pr-2.5 py-0.5", status.bg, status.color)}>
                ...
                <div className="flex items-center justify-between border-t border-zinc-800/50 pt-3 mt-1">
                  ...
                  {/* Admin Response Preview if exists */}
                ...
                {/* Admin Response Expanded Block */}
                {feedback.adminResponse && (
                  <div className="mt-3 rounded-lg bg-zinc-950 border border-zinc-800/60 p-3">
