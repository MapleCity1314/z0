...
export function resolveTwitterCommand(packageRoot, env = process.env) {
  ...
  if (sourceRoot && existsSync(sourceRoot) && commandExists("uv")) {
    return {
      command: "uv",
      args: ["run", "--project", sourceRoot, "twitter"],
      cwd: packageRoot,
    };
  }
  ...
  if (sourceRoot && existsSync(sourceRoot)) {
    return {
      command: env.Z0_TWITTER_PYTHON_BIN || "python3",
      args: ["-m", "twitter_cli.cli"],
      cwd: sourceRoot,
    };
  }
...
export function executeTwitterCommand(packageRoot, cliArgs, env = process.env) {
  let resolved;

  try {
    resolved = resolveTwitterCommand(packageRoot, env);
  } catch (error) {
    return {
      error: true,
  ...
  if (result.status !== 0) {
    return {
      error: true,
      message: (result.stderr || result.stdout || "").trim() || "twitter-cli execution failed",
