...
test("twitter MCP lists tools and returns a structured self-check result", async () => {
  const child = spawn(process.execPath, [resolve(process.cwd(), "bin/z0-twitter-mcp.mjs")], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      PATH: "",
      Z0_TWITTER_SOURCE_ROOT: "__missing__",
    },
  ...
  child.stdin.write(
    encodeMessage({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "twitter_runtime_self_check", arguments: {} },
    }),
  );
  ...
  assert.ok(toolsList);
  assert.ok(
    toolsList.result.tools.some((tool) => tool.name === "twitter_runtime_self_check"),
  );
