...
function binaryNameForPlatform(platform = process.platform) {
  return platform === "win32" ? "z0-xiaohongshu-cli.exe" : "z0-xiaohongshu-cli";
}
...
export function resolveNativeBinary(packageRoot, env = process.env) {
  const overridePath = ensureAbsolutePath(process.cwd(), env.Z0_XIAOHONGSHU_CLI_BIN);
  const candidates = [
    overridePath,
  ...
  throw new Error(
    [
      ...
      "Run `pnpm --filter @z0/xiaohongshu build` or set `Z0_XIAOHONGSHU_CLI_BIN`.",
