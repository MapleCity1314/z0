...
function binaryNameForPlatform(platform = process.platform) {
  return platform === "win32" ? "z0-bilibili-cli.exe" : "z0-bilibili-cli";
}
...
export function resolveNativeBinary(packageRoot, env = process.env) {
  const overridePath = ensureAbsolutePath(process.cwd(), env.Z0_BILIBILI_CLI_BIN);
  const candidates = [
    overridePath,
  ...
  throw new Error(
    [
      "No Bilibili CLI binary was found.",
      "Run `pnpm --filter @z0/bilibili build` or set `Z0_BILIBILI_CLI_BIN`.",
...
export function stageNativeBinary(packageRoot, options = {}) {
  ...
  if (!existsSync(sourcePath)) {
    throw new Error(`Bilibili native binary not found at ${sourcePath}. Build the native target first.`);
  }
