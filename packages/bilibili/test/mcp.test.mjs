...
test("bilibili MCP lists tools and returns a structured self-check result", async () => {
  const child = spawn(process.execPath, [resolve(process.cwd(), "bin/z0-bilibili-mcp.mjs")], {
    ...
    env: {
      ...process.env,
      PATH: "",
      Z0_BILIBILI_SOURCE_ROOT: "__missing__",
    },
  ...
  child.stdin.write(
    encodeMessage({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "bilibili_runtime_self_check", arguments: {} },
    }),
  );
  ...
  assert.ok(toolsList);
  assert.ok(
    toolsList.result.tools.some((tool) => tool.name === "bilibili_runtime_self_check"),
  );
