...
interface VersionItemProps {
  version: Omit<VersionRecord, "createdAt" | "updatedAt" | "publishedAt"> & {
    createdAt: Date;
    updatedAt: Date;
...
export function VersionItem({
  ...
  const typeConfig =
    TYPE_CONFIG[version.type as keyof typeof TYPE_CONFIG] || TYPE_CONFIG.patch;
  const isLatest = version.isLatest;
  const dateObj = version.publishedAt
            ...
            <div className="flex-1 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {isLatest && (
                  <Badge
                    variant="outline"
                    className="text-[10px] px-1.5 py-0 h-5 bg-blue-500/10 text-blue-400 border-blue-500/20 flex items-center gap-1 animate-pulse"
                  >
                    ...
                    <div className="mt-6 pt-4 border-t border-zinc-800/50 flex flex-col sm:flex-row gap-4 sm:items-start justify-between">
                      {version.migration && (
                        <div className="flex-1 bg-zinc-950/50 rounded-lg p-3 border border-zinc-800">
                          ...
                          <p className="text-xs text-zinc-500 font-mono whitespace-pre-wrap">
                            {version.migration}
                          </p>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {version.docsUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener"
                            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors"
                          >
                        ...
                        {version.downloadUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener"
                            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium bg-zinc-100 text-zinc-900 hover:bg-white transition-colors"
                          >
